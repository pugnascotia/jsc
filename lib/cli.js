...
function expandPaths(inputPaths) {
  ...
  var promises = inputPaths.map(function(pattern) {
...
`, {
  ...
  expandPaths(cli.input)
    .then(function (allFiles) {

      if (allFiles.length === 0) {
        console.warn(chalk.red('No files to process or files don\'t exist'));
        process.exit(1);
      }

      var config = normalizeConfig(cli.flags);

      compiler.compile(allFiles, config, function(err) {
        if (err) {
